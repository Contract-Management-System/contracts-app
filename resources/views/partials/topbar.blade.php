...
<header class="h-16 bg-white border-b border-slate-100 flex items-center px-6 gap-4 shrink-0">
    ...
    {{-- Page title (set with @section) --}}
    <div class="flex-1 min-w-0">
        <h1 class="text-sm font-semibold text-slate-900 truncate">
            @yield('page-title', 'Dashboard')
        </h1>
        @hasSection('page-breadcrumb')
        <p class="text-xs text-slate-400 truncate">@yield('page-breadcrumb')</p>
        @endif
    </div>
    ...
    <div class="flex items-center gap-2">
        ...
        {{-- Add contract quick action --}}
        <a href="{{ route('contracts.create') }}" class="btn-primary btn-sm hidden sm:inline-flex">
